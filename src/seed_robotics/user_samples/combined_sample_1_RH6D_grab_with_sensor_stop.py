...
# ID = 0 -> thumb flexion
# ID = 1 -> index flexion
# ID = 2 -> middle finger flexion (Here the exemple is for a RH6D so no index finger)
...
def mainBoardCallback(main_board_data):
    control.IR_sensor_value = main_board_data.boards[0].palm_IR_sensor
...
def sensorCallback(sensor_data):
    # If the sensors data are from a right hand, discard the information
    # Because this sample is for a left hand
    if sensor_data.hand_polarity == "right":
        return
    else:
...
rospy.init_node('User_Sample_Listener', anonymous = True)
...
rospy.Subscriber("Joints", AllJoints, jointsCallback)
...
rospy.Subscriber('Main_Boards', AllMainBoards, mainBoardCallback)
...
rospy.Subscriber('AllSensors',AllSensors,sensorCallback)
...
pub = rospy.Publisher('speed_position', JointListSetSpeedPos, queue_size = 10)
    ...
    for sensor in sensor_list:
        if sensor.abs > SENSOR_FORCE_LIMIT:
            ...
            # If that joint is not already in the list of stressed joint, find it in joint_list
                ...
                print("Stopping joint %s with sensor threshold" % joint_name)
            # Then set its target position to its current position
                stopStressing(corresponding_joint[0])
                list_stressed_joints.append(corresponding_joint[0].name)
